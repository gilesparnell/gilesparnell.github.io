...
- [x] ~~User to decide content for `Claude/{skills,walkthroughs,templates}.html`~~ **RESOLVED 2026-07-23 (Giles):** Knowledge stays in the separate `gilesparnell/Claude` repo (it owns the live `/Claude/` path via project-site precedence and has the self-rebuilding portal Action). The shell pages were **deleted from this repo** — do not recreate them. NEW open item below.
- [ ] **Restyle the Knowledge portal to 1C Console in the `gilesparnell/Claude` repo** (its own templates/`portal.css` + the `build-portal.yml` Action templates) — Claude Design. Do not add `Claude/` pages to this repo.
- [ ] **Generator guarded (Claude Code, 2026-07-23):** `scripts/build-project-pages.js` still emits the old Deep Ocean template, so `build()` now throws (`TEMPLATE_IS_STALE = true`) to stop anyone regenerating over the deployed Console pages — the risk is real: running it writes 11 old-theme files. When landing the Console template + `projects.data.js` split, set `TEMPLATE_IS_STALE = false` and update the guard tests at the end of `scripts/build-project-pages.test.js`.
